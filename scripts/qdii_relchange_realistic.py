...
def main():
    ...
    L.append("\n## 5. 关键结论\n")
    L.append("- **实盘约束不杀信号，只杀「高换手高溢价」的纳指**：标普/德国/中概的净超额几乎完整保留甚至微升（标普 +16.67→+15.32、德国 +14.02→+15.94、中概 +9.08→+9.38），纳指缩水 60%（+22.37→+8.97），日经转负（+3.40→−0.78）。原因：纳指溢价波动最大、换手最高（理想 142 次），成本 + min_hold 约束的杀伤集中落在它身上。")
    L.append("- **触发前提 floor 净效果为正**：floor=0→3% 组合净超额 +8.5→+5.6 平缓单调、无悬崖；floor 抬到 1% 过滤低溢价噪声触发，标普/德国因此净超额不降反微升。")
    L.append("- **最小持有期有效压降换手**：min_hold=5 把纳指换手 142→110、恒生 142→44（腰斩），年单边换手从约 17 次降到 4~6 次，贴合 QDII T+2 到账约束；min_hold 1→20 组合净超额 +8.5→+4.2 平缓衰减。")
    L.append("- **成本敏感性温和，信号并非「薄利到碰不得」**：cost 0→0.30% 组合净超额仅 +10.1→+6.6（降 3.5pp），0.15% 合理成本下组合净超额 +8.4pp，仍显著为正。")
    L.append("- **操作含义（修正上一轮判断）**：上一轮「收益被高估 2-3 倍」的说法偏悲观——实盘约束下真实净超额是「纳指被高估、其余基本实」。组合平均净年化超额 +8.4pp，纳指 +8.97、标普 +15.32、德国 +15.94，仍可作为「减仓/对冲」信号叠加；但必须容忍 QDII 限购导致「想买回时买不回」的执行风险，且 2026 年信号衰减趋势未变。")
