...
def main():
    ...
    L.append("| 代码 | 名称 | 市场 | 最新价 | 官方IOPV | 官方溢价% | 底层涨跌% | 影子IOPV | 影子溢价% | 信号 |")
    L.append("|---|---|---|---|---|---|---|---|---|---|")
    for _, r in df.iterrows():
        L.append(
            f"| {r['code']} | {r['name']} | {r['market']} | {fmt(r['price'])} "
            f"| {fmt(r['iopv_official'], 4)} | {fmt(r['official_premium_pct'])} "
            f"| {fmt(r['underlying_chg_pct'])} | {fmt(r['shadow_iopv'], 4)} "
            f"| {fmt(r['shadow_premium_pct'])} | {r['alert']} |"
        )

    L.append("\n## 说明\n")
    L.append("- **官方溢价** = −东财「基金折价率」（正值=溢价、负值=折价），是交易所盘中 IOPV 口径。")
    L.append("- **影子溢价** 把底层市场（美股/港股）最新一跳折进 IOPV，更接近真实净值；美股 QDII 在 A 股盘中官方 IOPV 滞后，影子口径更准。")
    L.append("- 德国(DAX)/日本(日经)暂无可靠日线源，影子列留空（退回官方口径）。")
    ...
    print(df[["code", "name", "price", "iopv_official", "official_premium_pct",
              "shadow_premium_pct", "alert"]].to_string(index=False))
