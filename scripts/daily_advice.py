...
def solve_lots(positions: dict[str, int], prices: dict[str, float],
    ...
    为什么不用贪心：`paper_trading.py reconcile` 对买卖腿分别 floor 到整手，
    两边同时向下取整导致「卖出回款 > 买入支出」，多余现金溢出（实测现金
    16.1%→24.2%、总偏离 28.4pp）。而「按缺口比例分配回款」的贪心版本在只有
    单个买入腿时会吃掉全部回款造成超买（缺口 4.7 万却买 5.6 万）。
