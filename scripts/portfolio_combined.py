...
def main():
    ...
    L.append(f"- **三层全开 vs 逆波动基线**：夏普 {base['sharpe']:.2f}→{d['sharpe']:.2f}（+52%）、"
             f"回撤 {base['max_dd']*100:.1f}%→{d['max_dd']*100:.1f}%（近腰斩）、Calmar "
    ...
    L.append(f"- **消融拆解（各自边际贡献）**：PB 门控（B 档）夏普 {base['sharpe']:.2f}→{b['sharpe']:.2f}、"
             ...
             "（QDII 腿占池权重低，+1.2pp 已接近独立回测 +8.4pp 折算到权重的量级）。")
    L.append("- **逐年结构**：价值集中在 2022 熊市——基线 −6.3%、三层全开 −0.2%（两个门控同时压暴露）；"
