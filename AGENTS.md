...
10. **整数手调仓逻辑只写在 `scripts/rebalance_solver.py`**：`daily_advice.py`
    ...
    求解器为 **v2 MILP（2026-09-03 起）**：scipy/HiGHS 精确求解 L1 偏离，替代原
    `itertools.product` 暴力枚举（后者在 18 腿下会 3e8 组合爆炸，且 `min_dev`
    前置过滤候选腿会**死锁**——单一超配腿 + 全部对手腿低配时目标函数退化成常数，
    最优解取 0 交易，21.8pp 偏离原地锁死）。`min_dev` 现仅作档位阶梯起点，
    自动放宽至全局最优。改阈值语义或目标函数前先读该文件顶部 docstring
    「两代算法」一节；MILP 的 **方向指示变量 y_j 不可删**，否则优化器会用
    「同腿同时买卖虚付佣金」的方式骗取现金偏离改善（实测报出 12.90pp 假解）。
    （v2.2 起该角色由 `zb/zx` 承担，见下，别再改回 `y_j`。）
    **v2.1 修正（2026-09-04）**：档位选择与 `MIN_GAIN_PP` 判定顺序曾互相打架——
    「不劣于最优 +0.3pp 的最严档位」会选中一个**改善为 0** 的档位，随后门槛又拿
    这个档位去比对，于是全局最优那 0.1pp 真实改善被白白放弃（实测 09-04 报
    「无需调仓」，实际存在 3.3→3.2pp 的 2 笔解）。现改为：**门槛一律以全局最优
    `best_dev` 判定**，并把「至少改善 `MIN_GAIN_PP`」作为档位选择的上界，
    两者不再互相否定。
    **v2.2 重写（2026-09-24）**：v2.1 的「回选最严档位」整体作废，理由与做法见
    该文件 docstring「v2.2」一节，要点：
    - 更严档的可行域 ⊆ `lvl=0` 的可行域 ⇒ **`lvl=0` 的 MILP 最优偏离必然最小**。
      「更严档 = 更少下单」是**无根据的假设**（MILP 在档位内部只最小化偏离、
      从不最小化笔数）。2026-09-21 反例：更严档 4 笔 / 换手 1,427 / 2.7834pp
      vs `lvl=0` 的 3 笔 / 换手 779 / 2.7825pp，三项全面更优却被舍弃。
    - 容差 `LEVEL_TOL_PP`（0.3pp ≈ 600 元跟踪误差）改名 `DEV_TIE_PP = 0.02pp`，
      且**必须 < `MIN_GAIN_PP`**，否则会用容差绕过净改善门槛。
    - 新增**两阶段 MILP**：第二阶段固定 `Σd + dc ≤ ceil_dev`、改最小化笔数，
      得到严格意义上并列解里笔数最少者。**方向指示由 v2 的 y_j 换成 zb/zx**
      （`zb + zx ≤ 1`），语义更直接且能承载最小交易额；**别再改回 y_j**。
    - 最终排序键 `(笔数, 偏离, 换手金额)`：笔数优先于偏离（容差 0.02pp 仅约
      40 元配置偏差，少一笔单省的是操作负担）；偏离优先于换手（换手 4,000 元
      只省约 6 元佣金）。
    - 新增**单腿最小交易额** `MIN_TRADE_CNY`，**默认 0（关闭）**——实测为净负
      收益（详见 docstring）。仅当券商收最低佣金 C 时才设 `C / 实际费率`。
...
1. **PB 门控 shift(1)**：t 月实际应用的是 t−1 月末分位（比直觉多滞后一个月）。
   乐咕数据月初才更新上月末点，实盘天然如此；回测 1.63 夏普就是这个口径跑出来的。
   **不要"修正"成当月分位**。
