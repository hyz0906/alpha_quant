...
2. **QDII 套利后续增强**（产品化已落地，此为可选优化）：盘中实时轮询（当前为
   ...
   §7.18 G 实盘约束版回测）
3. **组合实盘化**：`portfolio_combined.py` 的三层组合（§7.20）接入月频再平衡
   调度 + 每日 QDII 门控刷新（可与 `qdii_daily.py` 定时任务合并），作为
   AlphaQuant 正式组合输出；换手/成本监控已在回测内（3.45 次/年 @ 0.15%/边）
