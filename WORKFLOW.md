...
链路、滑动窗口对照、参数扫描、显著性检验、**扩展池验证**均已完成（§7.4~§7.7）。
当前结论（已翻案）：**在 12 只 ETF / 13 窗 / 含 2018 与 2022 下跌的扩展池上，
所有 RSRS 主动策略全部跑输等权被动组合**——周频版被 6.69% 换手成本吃掉正毛收益、
日频版显著为负（t=−3.00）。§7.6 的「周频版唯一候选」是 3 ETF 纯牛市样本的
选择偏差。下一步按优先级：

1. **降频 + 加空仓过滤（最高优先）**：成本归因证明周频版毛收益 +3.54% 尚可，
   亏损主因是 152.88 总换手 → 6.69% 成本。先试月频调仓 / 加 min_score 门槛 /
   调仓容忍带（rebalance_tolerance），把换手压到日频版 1/4 以下再复测
2. **因子有效性诊断**：RSRS 在扩展池上毛收益仅 +3.54%（6 年），远低于等权
   +77.63%，说明信号本身偏弱。需检验 RSRS zscore 在 12 只池上的截面单调性
   （IC / 分层收益），判断是因子失效还是池子同质化（PC1=57.5%）
3. 换手成本敏感性：当前 commission 0.025%/笔，回测未计冲击成本与滑点；
   12 只池上日频版 8% 超额胜率说明成本敏感度极高，实盘需叠加 ETF 申赎费率
4. QDII 影子 IOPV 实装（`qdii_calc.py` 仍是 mock，premium=当日涨跌幅）
5. 若降频后仍不占优，接受「等权/风险平价被动为基线」结论，把精力转向
   另类因子（黄金 518880 是池内唯一真独立标的 ρ≈0.02~0.09，值得单列研究）
