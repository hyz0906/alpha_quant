...
1. **基本面全历史截面 IC（最高优先）**：Tushare token 已配置但积分为 0，
   连基础接口都无权限（40203）。需用户在 tushare.pro 补够 2000 积分，之后即可
   拉 `index_dailybasic` 跑多年 × 多标的截面 IC（骨架已在 `fundamental_loader.py`
   的 `TS_INDEX_MAP` + `fetch_tushare_index_dailybasic()`）；当前已用乐咕 3 宽基
   月频做了时序择时替代（§7.16 + §7.19 完整回测），且用蛋卷公开接口补齐了
   63 指数单截面估值排名（§7.17）。**另一条免费路**：蛋卷 `history` 接口（63 指数 × 2016 至今日频
   PE/PB/分位）仅需雪球/蛋卷登录 cookie 即可解锁，若用户提供 cookie，可完全绕开
   Tushare 拿历史面板跑标准截面 IC
