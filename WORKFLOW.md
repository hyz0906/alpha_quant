...
链路、滑动窗口对照、参数扫描、显著性检验、扩展池验证、降频验证均已
完成（§7.4~§7.8）。当前结论（已收口）：**RSRS 截面轮动在扩展池上被彻底
证伪**——日/周/月三档频率、top_k 1/2/4/6/8、多档 min_score 全部跑输等权
被动组合。成本归因 + 降频验证进一步证明这是**信号 alpha 不足**（周频毛收益
天花板 +56.63% < 等权 +66.48%，零手续费也跑不赢），而非单纯换手成本问题。
剩余方向按优先级：

1. **因子有效性诊断（最高优先）**：RSRS 在 12 只池上截面区分度不足。检验
   zscore 的截面单调性（IC / 分层收益 / 多空收益），判断是因子本身失效、
   还是池子同质化（PC1=57.5%）稀释了截面差异。若是后者，可尝试在更分散的
   池（如纳入黄金 518880 等真独立标的加权）上复测
2. **换手成本敏感性建模**：当前 commission 0.025%/笔且未计滑点/冲击成本，
   12 只池上周频版 156 次/窗调仓的实盘成本只会更高。若继续研究主动策略，
   必须先把成本模型做实（ETF 申赎费率 + 盘中滑点）
3. 接受「等权/风险平价被动为基线」结论：把精力转向另类因子（黄金 518880
   是池内唯一真独立标的 ρ≈0.02~0.09，值得单列研究）或 QDII 溢价套利
