...
诊断、新因子批量筛选、**异构池检验**均已完成（§7.4~§7.11）。**最终结论
（收口）**：RSRS + 16 个标准价量因子在**同质池与异构池上全部失效**——
最高 |ICIR|=0.289（12-1 动量），卡在 0.3 门槛之下。§7.10「根因是池子同质化」
的假设被 §7.11 证伪：**即便跨资产/跨市场异构化（债券+商品+5 区域跨境权益），
日频价量截面因子依然没有可交易 alpha**。价量数据的截面 alpha 天花板已探明。
剩余方向按优先级：

1. **转向时序/轮动信号（最高优先）**：截面价量因子已死，但「股金轮动」
   「趋势择时」「波动率择时」「大类资产动量配置」等时序信号不依赖截面
   区分度，且异构池（518880 黄金 ρ≈0.02~0.09、511010 国债、跨境权益）
   恰好提供了可轮动的独立腿。可直接在异构池上验证时序择时
2. **基本面/另类因子（需新数据源）**：carry（债券收益率、股息率、商品
   期限结构）、价值（估值分位）等因子需要价量之外的数据，是价量证伪后的
   自然出路；但需先接入新数据源（Tushare/akshare 财务数据）
3. **接受被动基线 + 风险平价**：把等权/风险平价做成正式组合（月度再平衡），
   作为 AlphaQuant 的兜底输出，同时量化最大回撤/换手成本
4. QDII 影子 IOPV 实装（`qdii_calc.py` 仍是 mock，premium=当日涨跌幅），
   为 QDII 溢价套利打基础——这是当前唯一有「结构套利」性质的独立 alpha
