...
诊断均已完成（§7.4~§7.9）。**最终结论**：RSRS 截面轮动作为核心主动策略
被证伪——截面 IC 均值 −0.01、ICIR < 0.05、分层收益无单调、多空收益趋零，
因子在扩展池上是纯噪声；三档频率 × 多档 top_k × 多档 min_score 全部跑输
被动等权。**被动等权是当前唯一站得住的基线。** 剩余方向按优先级：

1. **换新因子（最高优先）**：RSRS 已死，需换真正有截面区分度的因子。
   候选：动量（价格/成交额动量）、低波、量价背离、行业轮动（申万行业
   ETF 轮动）、或 QDII 溢价套利（当前 mock）。新因子接入前先跑 §7.9 的
   IC + 分层收益诊断，|ICIR| < 0.5 直接淘汰，避免重蹈 RSRS 覆辙
2. **接受被动基线**：把等权/风险平价做成正式组合（月度再平衡），解决
   再平衡成本与资金分配，作为 AlphaQuant 的兜底输出
3. **另类因子单列研究**：黄金 518880 是池内唯一真独立标的（ρ≈0.02~0.09），
   可研究「股金轮动」或作为组合的风险对冲腿
