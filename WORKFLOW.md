...
新因子批量筛选、异构池检验、**时序择时 + 大类资产轮动**均已完成
（§7.4~§7.12）。**最终结论（收口）**：价量数据在 ETF 宇宙上的三条主动 alpha
路线——截面因子（16 个 + RSRS）、时序趋势择时（多/空现金）、大类资产轮动
（切债/金/动量 top1）——**全部证伪**。修正轮动策略的前视偏差后，无任一主动
策略夏普超过被动等权基线（0.60）。**价量维度 alpha 天花板已彻底探明：≈0**。

剩余方向按优先级（全部跳出价量维度）：

1. **基本面/另类因子（最高优先，需新数据源）**：carry（债券收益率、股息率、
   商品期限结构）、价值（估值分位）等因子是价量证伪后唯一的「新 alpha 维度」。
   需先接入基本面数据源（Tushare/akshare 财务数据或东方财富接口）
2. **结构套利（次高）**：QDII 影子 IOPV 实装（`qdii_calc.py` 仍是 mock，
   premium=当日涨跌幅），为 QDII 溢价套利打基础——这是唯一有「结构套利」
   性质、不依赖预测的独立 alpha
3. **接受被动基线 + 风险平价（兜底）**：把等权/风险平价做成正式组合（月度
   再平衡；黄金 0.78 / 国债 1.37 是被动持有里夏普最高的两腿），作为 AlphaQuant
   的兜底输出，同时量化最大回撤/换手成本
