...
链路、滑动窗口对照、参数扫描、显著性检验均已完成（§7.4~§7.6）。
当前结论：**周频版（rsrs_rotation_weekly, k2/ms0.5）是唯一保留的候选**
（Sharpe 1.70、超额 p<0.10，但未达 95% 显著）；timing 已淘汰；
k1 高收益高波动、Sharpe 与等权持平。下一步按优先级：

1. **扩大候选池 + 延长历史（最高优先）**：这是突破样本限制的唯一途径。
   换 510300 / 518880 / 513100 等 2015 年前上市的 ETF，数据前推到 2015，
   覆盖 2018 熊市与 2022 下跌，窗口数从 7 增至 20+，配对检验功效才够
2. **样本外验证**：周频版参数冻结，在 2026H2 之后行情前滚跟踪；
   或在新候选池上直接复用 k2/ms0.5/周频，看是否复现超额
3. 换手成本敏感性：当前 commission 0.025%/笔，验证周频版在更高费率下
   是否仍占优（其换手低，应比日频版更稳健）
