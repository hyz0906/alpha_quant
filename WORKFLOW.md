...
链路已跑通，滑动窗口对照与第一轮参数扫描也完成了（§7.4/§7.5）。
结论：**默认参数跑输被动等权，但调参后（top_k=1 或周频调仓）能跑赢**；
周频版（夏普 1.51、超额胜率 71%）是当前最优候选。下一步按优先级：

1. **显著性检验（最高优先）**：扫参最优可能是选择偏差。用 vibe 自带的
   `monte_carlo_test` / `bootstrap_sharpe_ci` / `walk_forward_analysis`
   检验周频版与 top_k=1 版的超额是否统计显著
2. **样本外验证**：用最优参数在 2026H2 之后的行情前滚验证（等数据积累），
   或前推数据起点——注意 588000 2020-11 才上市，要提前需换候选池
   （如 510300/518880/513100，均有 2015 前历史，可覆盖 2018/2022 熊市）
3. 扩大候选池（当前 3 只 ETF，截面轮动选择空间太窄；
   STRATEGY_PLAN.md §4 有候选池扩展设计）
4. 换手成本敏感性：当前 commission 0.025%/笔，验证结论在更高费率下
   是否仍成立（周频版换手低，应更稳健）
