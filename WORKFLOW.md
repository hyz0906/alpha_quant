...
**基本面时序估值择时（§7.16）**、**蛋卷 63 指数截面估值排名（§7.17）**、
**QDII 溢价实装（§7.14）**、**QDII 溢价套利回测 + 产品化（§7.18）**、
**风险平价兜底**均已完成（§7.4~§7.18）。**最终结论
（收口）**：价量数据在 ETF 宇宙上的三条主动 alpha 路线——截面因子（16 个 +
RSRS）、时序趋势择时（多/空现金）、大类资产轮动（切债/金/动量 top1）——
**全部证伪**（天花板≈0）。落地成果：

1. **基本面/估值因子**：快照诊断（§7.13）+ 时序估值择时检验（§7.16）已完成。
   沪深300/上证50 的 value 因子时序有效且跨时段稳健（多空 19~23%），中证500
   无效。截面 IC 仍受限于「多年 × 多标的」估值数据缺口（Tushare 需 2000 积分）。
...
1. **基本面全历史截面 IC（最高优先）**：Tushare token 已配置但积分为 0，
   ...
   月频做了时序择时替代（§7.16），且用蛋卷公开接口补齐了 63 指数单截面估值排名
   （§7.17）。**另一条免费路**：蛋卷 `history` 接口（63 指数 × 2016 至今日频
