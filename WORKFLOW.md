...
约 20 日 PE + 股息率（快照）；乐咕乐股 `stock_index_pe_lg/pb_lg` 给约 1 年
日频 PE/PB（短窗分位）。7 只 A 股权益 ETF 中 6 只拿到 PE + 股息率（创业板
399006 因中证官网 404 缺失）。
...
⚠️ **关键局限（诚实记录）**：免费 akshare **无多年点-in-time 估值历史**，
只有「快照 + 1 年短窗」，**无法跑截面 IC 时间序列检验**（需因子 × 未来收益
多年面板）。补全路径：Tushare Pro `index_dailybasic`（需 token）或中证指数
官网历史导出。本项落地的是「基础设施 + 快照 + 覆盖矩阵」，全历史 IC 检验
是明确的下一项数据接入任务。
...
**QDII 溢价实装**、**风险平价兜底**均已完成（§7.4~§7.15）。**最终结论
（收口）**：价量数据在 ETF 宇宙上的三条主动 alpha 路线——截面因子（16 个 +
RSRS）、时序趋势择时（多/空现金）、大类资产轮动（切债/金/动量 top1）——
**全部证伪**（天花板≈0）。落地成果：

1. **基本面/估值因子**：已建基础设施 + 快照诊断（§7.13），但全历史 IC 检验
   受限于免费源无多年点-in-time 估值数据。
...
1. **基本面全历史数据接入（最高优先）**：接 Tushare Pro `index_dailybasic`
   （需 token）或中证指数官网历史导出，补上多年点-in-time 指数 PE/PB/股息率，
   才能对估值/股息率因子跑标准截面 IC 检验（§7.13 已明确此缺口）
