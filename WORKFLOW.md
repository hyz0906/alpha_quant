...
ConnectionError，代码已优雅降级退回官方口径；德国 DAX/日经暂无可靠日线源，
影子列留空。
...
- QDII 影子 IOPV 已实装真实 IOPV/折价率 + 美股/港股影子调整（§7.14），但
  德国 DAX/日经暂无可靠日线源（影子列留空）、汇率日内变动未计，仍是一阶近似
...
**QDII 溢价实装**、**风险平价兜底**均已完成（§7.4~§7.17）。**最终结论
...
2. **QDII 溢价套利**：真实 IOPV/折价率 + 影子调整已实装（§7.14），抓到纳指/
   标普 500 近 10% 溢价的真实套利信号——当前唯一站得住的独立 alpha。
...
2. **QDII 溢价套利产品化**：把 `qdii_monitor.py` 接入定时任务（每日收盘/盘中
   轮询），补德国 DAX/日经底层源 + 汇率日内变动，做影子 IOPV 的精细版；
   回测「溢价>3% 回避、折价买入」的套利收益
