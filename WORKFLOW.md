...
- **floor 净效果为正**：0→3% 组合净超额 +8.5→+5.6 平缓单调无悬崖，floor=1% 过滤
  低溢价噪声后标普/德国净超额不降反微升。
- **min_hold 有效压降换手**：纳指 142→110、恒生 142→44（腰斩），年单边换手从 ~17
  次降到 4~6 次；min_hold 1→20 净超额 +8.5→+4.2 平缓衰减。
...
保护形态；④ 敏感性稳健：PB 规则 × 范围 × 成本共 8 变体夏普 1.11~1.43 全部 >1
（binary 样本内最高但不据此调参）。

**踩坑**：复用 `qdii_backtest.load_premium_history` 时缓存键是**裸代码**（513100），
池代码带 `.SH` 后缀须 `split(".")[0]` 剥离，否则缓存未命中走网络重拖。
